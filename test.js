...
var additionalTlds = ['faketld', 'phonytld'];

var matchedFixtures = tlds.map(function (tld) {
	return 'google.' + tld;
});

matchedFixtures = matchedFixtures.concat([
...
test('match URLs', function (t) {
	var fixtures = matchedFixtures.concat(tlds.map(function (tld) {
		return '"google.' + tld + '"';
	}));
...
test('match URLs (additionalTlds)', function (t) {
	var fixtures = matchedFixtures.concat(tlds.map(function (tld) {
		return '"google.' + tld + '"';
	}));

	fixtures = fixtures.concat(additionalTlds.map(function (tld) {
		return 'google.' + tld;
	}));
