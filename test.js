...
test('match liberal', t => {
	...
	fixtures.forEach(x => t.true(fn({exact: true, liberal: true}).test(x)));
});

test('do not match liberal', t => {
	const fixtures = [
	...
	fixtures.forEach(x => t.false(fn({exact: true, liberal: true}).test(x)));
});
